  ...
  getData: async function (req) {
    ...
    try {
      let query = supabase
        .from('orders_view')
        .select(
          'id, number, user_id, status, placed_at, total_cents, currency',
          { count: 'exact' }
        );
      dbStats.increment();
      ...
      await Promise.all(
        rows.map(async (c) => {
          c.email = await cache.wrap(
            'user:email:' + c.user_id,
            TTL,
            async () => {
              try {
                const { data: listData, error: userErr } =
                  await supabase.auth.admin.getUserById(c.user_id);
                dbStats.increment();
                if (userErr) {
                  debug.error(
                    'Error fetching user for order listing:',
                    userErr
                  );
                  return null;
                }
      ...
      const orders = (rows || []).map((r) => ({
        ...r,
        placed_at_display: r.placed_at
          ? new Date(r.placed_at).toLocaleDateString()
          : '',
...
bind(router, {
  ...
  getData: async function (req, res) {
    ...
    try {
      ...
      row.email = await cache.wrap(
        'user:email:' + row.user_id,
        TTL,
        async () => {
          try {
            const { data: listData, error: userErr } =
...
router.post(
  ...
  async (req, res) => {
    const { id } = req.params;
    const status =
      req.body && req.body.status ? String(req.body.status).trim() : '';
    const supabase = masterClient();
    ...
    try {
      ...
      if (req.session)
        req.session.flash = { error: 'Failed to update order status.' };
      return res.redirect('/admin/orders');
    }
  }
);
