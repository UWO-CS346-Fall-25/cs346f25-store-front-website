    ...
    try {
      ...
      if (!q) {
        ...
        if (error) {
          ...
          return {
            ...
            paging: { page, perPage, totalPages: 1 },
            q,
          };
        }
      ...
      while (shouldContinue) {
        ...
        if (batch.length < fetchPerPage) {
          ...
          const totalPagesRemote = Math.max(
            1,
            Math.ceil(data.total / fetchPerPage)
          );
      ...
      const filtered = allUsers.filter((u) => {
        ...
        const meta = u.user_metadata
          ? JSON.stringify(u.user_metadata).toLowerCase()
          : '';
        return (
          email.includes(qlc) || display.includes(qlc) || meta.includes(qlc)
        );
      });
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (targetRole === 'admin') {
        if (req.session)
          req.session.flash = {
            error: 'Modifying admin accounts is not allowed.',
          };
        return res.redirect('/admin/users');
      }
      ...
      if (req.session) {
        req.session.flash = {
          error: error ? 'Failed to update role.' : null,
          success: error ? null : 'Role updated.',
        };
      }
      return res.redirect('/admin/users');
    } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (req.user && String(req.user.id) === String(id)) {
        if (req.session)
          req.session.flash = { error: 'You cannot ban or unban yourself.' };
        return res.redirect('/admin/users');
      }
      ...
      if (action === 'ban') {
        ...
        if (req.session)
          req.session.flash = {
            error: error ? 'Failed to ban user.' : null,
            success: error ? null : 'User banned.',
          };
        ...
        if (req.session)
          req.session.flash = {
            error: error ? 'Failed to unban user.' : null,
            success: error ? null : 'User unbanned.',
          };
      } else {
