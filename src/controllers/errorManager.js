...
function passwordChecker(object, { newAccount = false, currentPassword, newPassword, confirmPassword, email }) {
  if (!newAccount) {
    if (!currentPassword) addError(object, 'Current password is required', 'currentPassword');
  } else {
  ...
  if (newPassword && newPassword.length < 8) {
    addError(object, 'New password must be at least 8 characters', 'newPassword');
  }
  if (!confirmPassword) addError(object, 'Please confirm your new password', 'confirmPassword');
