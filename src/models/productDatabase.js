...
function generateUrl(pid, path, external_url) {
  if (path != null) {
    const { data } = supabase.storage
      .from('product-images')
      .getPublicUrl(`products/${pid}/${path}`);
    ...
    products.map(async (p) => {
      ...
      p.images = await cache.wrap(key, TTL, async () => {
        const res = await database.query(
          'SELECT path, external_url, alt FROM public.product_images WHERE product_id = $1 ORDER BY is_primary DESC, id ASC',
          [p.id]
        );
        dbStats.increment();
  ...
  await Promise.all(
    products.map(async (p) => {
      ...
      p.image = await cache.wrap(key, TTL, async () => {
        const prodImg = await database.query(
          'SELECT path, external_url, alt FROM public.product_images WHERE product_id = $1 AND is_primary = true LIMIT 1',
          [p.id]
        );
        dbStats.increment();
  ...
  await Promise.all(
    products.map(async (p) => {
      ...
      p.categories = await cache.wrap(key, TTL, async () => {
        const res = await database.query(
          'SELECT c.id, c.name, c.slug FROM public.categories c JOIN public.product_categories pc ON c.id = pc.category_id WHERE pc.product_id = $1',
          [p.id]
        );
        dbStats.increment();
        return res.rows;
  ...
  await Promise.all(
    categories.map(async (c) => {
      ...
      c.products = await cache.wrap(key, TTL, async () => {
        const res = await database.query(
          'SELECT p.* FROM public.products p JOIN public.product_categories pc ON p.id = pc.product_id WHERE pc.category_id = $1',
          [c.id]
        );
        dbStats.increment();
        return res.rows;
...
async function getAll() {
  ...
  return cache.wrap(key, TTL, async () => {
    const res = await database.query(
      "SELECT * FROM public.products WHERE status <> 'archived' ORDER BY created_at DESC"
    );
    dbStats.increment();
    return res.rows;
  });
...
async function getActive() {
  ...
  return cache.wrap(key, TTL, async () => {
    const res = await database.query(
      "SELECT * FROM public.products WHERE status = 'active'"
    );
    dbStats.increment();
    return res.rows;
  });
...
async function getFeatured() {
  ...
  return cache.wrap(key, TTL, async () => {
    const res = await database.query(
      "SELECT p.* FROM public.products p JOIN public.featured_products fp ON p.id = fp.product_id WHERE p.status= 'active' ORDER BY fp.position ASC"
    );
    dbStats.increment();
    return res.rows;
  });
...
async function getActiveCategories() {
  ...
  return cache.wrap(key, TTL, async () => {
    const res = await database.query(
      'SELECT * FROM public.categories WHERE is_active = true ORDER BY position'
    );
    dbStats.increment();
    return res.rows;
  });
  ...
  return cache.wrap(key, TTL, async () => {
    ...
    if (isNaN(Number(idOrSlug))) {
      res = await database.query(
        'SELECT * FROM public.categories WHERE slug = $1',
        [idOrSlug]
      );
    } else {
      res = await database.query(
...
async function getNewArrivals(count) {
  ...
  return cache.wrap(key, TTL, async () => {
    const res = await database.query(
      "SELECT * FROM public.products WHERE status= 'active' ORDER BY created_at DESC LIMIT $1",
      [count]
    );
    dbStats.increment();
    return res.rows;
...
async function getArchived() {
  ...
  return cache.wrap(key, TTL, async () => {
    const res = await database.query(
      "SELECT * FROM public.products WHERE status = 'archived' ORDER BY updated_at DESC"
    );
    dbStats.increment();
    return res.rows;
  });
...
async function uncacheCategory(id, slug = null) {
  if (!slug) {
    const res = await database.query(
      'SELECT slug FROM public.categories WHERE id = $1',
      [id]
    );
    dbStats.increment();
    slug = res.rows[0]?.slug;
